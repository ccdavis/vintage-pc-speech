/* ESPKD: resident eSpeak NG voice for DOS, a "shell wrapper" DPMI process (DJGPP + CWSDPMI).
 *
 *   ESPKD [COMn] [/SPK] [/TEST] [/11K] [/PMALL] [/C command...]
 *
...
extern unsigned long engine_utterances, engine_aborted, engine_utt_start;
...
static int use_spk = 0, test_mode = 0, pm_all = 0, rate = 8000, nosti = 0;
...
#define SYNTH_STACK (96 * 1024)                     /* 47 KB measured; MakePhonemeList's 36 KB frame is the bulk */
...
static int lock_all(void)
{
    ...
    if (__dpmi_lock_linear_region(&m)) {
        printf("ESPKD: cannot lock %lu KB of memory (DPMI error %x): not enough free extended memory,\n"
               "       or a DPMI host that cannot lock; ESPKD needs about 1.5 MB of XMS.\n", m.size / 1024, __dpmi_error);
        return -1;
    }
    return 0;
}
...
static int install(void)
{
    ...
    if (dos_seg < 0) { printf("ESPKD: no DOS memory for the stub\n"); return -1; }
    ...
    if (_farpeekl(_dos_ds, ((unsigned long)old14_rm.rm_segment << 4) + RM_SIG) == 0x4B505345UL /* "ESPK" */) {
        printf("ESPKD: already installed (INT 14h points at an ESPKD stub); type EXIT first\n"); return -1;
    }
    ...
    if (_go32_dpmi_allocate_real_mode_callback_iret(&cb_put, &put_regs)) { printf("ESPKD: no real-mode callback\n"); return -1; }
    ...
    if (use_spk) {
        ...
        if (_go32_dpmi_allocate_real_mode_callback_iret(&cb_wake, &wake_regs)) { printf("ESPKD: no real-mode callback\n"); return -1; }
    ...
    if (!use_spk) {
        if (dsp_reset()) { printf("ESPKD: no Sound Blaster DSP at %Xh\n", sb_base); return -1; }
        ...
        {   /* DMA buffer: 2 * BUF of DOS memory, pick a block not crossing a 64 KB page */
            ...
            if (seg < 0) { printf("ESPKD: no DOS memory for the DMA buffer\n"); return -1; }
...
static void report(const char *tag)
{
    ...
    _farsetsel(_dos_ds);
    printf("%s: t=%lu imr=%02X irr=%02X isr=%02X sbint=%02X dmapos=%u sbirq=%lu rtc(pm=%lu rm=%lu) wake(pm=%lu rm=%lu) resumes=%lu int14(pm=%lu rm=%lu) ring=%lu/%lu text=%u/%u utt=%lu/%lu underrun=%lu flush=%lu stage=%u busy=%u\n",
           ...
           engine_utterances, engine_aborted, n_underrun, n_flush, synth_stage, synth_busy);
}

int main(int argc, char **argv)
{
    char dat[260], *p, *q, *comspec;
    ...
    for (i = 1; i < argc; i++) {
        ...
        else if (stricmp(argv[i], "/11K") == 0) rate = 11025;
        ...
        else { printf("usage: ESPKD [COM1..COM4] [/SPK] [/TEST] [/11K] [/PMALL] [/C command]\n"); return 1; }
    }
    /* ESPK.DAT next to the EXE */
    strncpy(dat, argv[0], sizeof(dat) - 10); dat[sizeof(dat) - 10] = 0;
    p = strrchr(dat, '\\'); q = strrchr(dat, '/'); if (q > p) p = q;
    ...
    if (_go32_dpmi_remaining_physical_memory() < 2000000UL) {   /* CWSDPMI dies with a page fault instead of failing malloc */
        printf("ESPKD: only %lu KB of extended memory is free; the voice needs about 2 MB (a RAM disk or EMS pool may be using it)\n",
               _go32_dpmi_remaining_physical_memory() / 1024);
        return 1;
    }
    heap0 = (unsigned long)sbrk(0);
    if (engine_init_espk(dat, rate)) return 1;
    engine_warmup();
    heap1 = (unsigned long)sbrk(0);
    if (lock_all()) return 1;
    if (install()) { uninstall(); return 1; }
    set_volume(5);
    if (use_spk) printf("ESPKD 0.1: eSpeak NG + Klatt at %d Hz on the PC speaker (RTC %u Hz PWM), DoubleTalk on COM%u; heap %lu KB locked, DOS free %u KB, XMS free %lu KB\n",
                        rate, SPK_RATE, port_index + 1, (heap1 - heap0) / 1024, dos_free_kb(), _go32_dpmi_remaining_physical_memory() / 1024);
    else printf("ESPKD 0.1: eSpeak NG + Klatt at %d Hz, DSP %d.%02d at %Xh IRQ %d DMA %d, DoubleTalk on COM%u; heap %lu KB locked, DOS free %u KB, XMS free %lu KB\n",
    ...
    send("\x01" "50P\x01" "5S\x01" "5V E speak ready.\r");
    ...
    setenv("PROMPT", "[E speak] $p$g", 1);            /* the child shell shows it is the talking shell; EXIT unloads */
    if (n_child) {
        char *av[36]; int k = 0;
        av[k++] = comspec; av[k++] = "/C";
        ...
        printf("Type EXIT to unload ESPKD.\n");
        r = spawnl(P_WAIT, comspec, comspec, NULL);
    }
    if (r < 0) perror("ESPKD: cannot run the shell");
    /* let the last words out (at most 2 s): text still queued, an utterance in progress, or samples not yet played */
    { unsigned long t0 = bios_ticks(); while (bios_ticks() - t0 < 36 && !(synth_waiting && text_head == text_tail && ring_committed <= ring_consumed)) ; }
    report("unload");
    uninstall();
    printf("ESPKD unloaded (%lu utterances, %lu heap allocations from interrupt context, synth stack %u bytes)\n", engine_utterances, guard_allocs, synth_stack_used());
    return r < 0 ? 1 : 0;
}
