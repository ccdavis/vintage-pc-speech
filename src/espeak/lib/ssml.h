/* SSML is compiled out in the ESPK subset: ProcessSsmlTag() is a stub that
 * ignores the tag (option_ssml is never set, so it is never reached). */
#ifndef ESPEAK_NG_SSML_H
#define ESPEAK_NG_SSML_H
#include <stdbool.h>
#include <wchar.h>
typedef struct {
        int tag_type;
