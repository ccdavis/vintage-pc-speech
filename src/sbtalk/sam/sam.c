...
char *input;   /* the caller's buffer (>= 256 bytes, reciter output, 155-terminated) */
//standard sam sound
...
long GetBufferLength(){return bufferpos;}
...
{
    Init();
    phonemeindex[255] = 255; /* permanent end marker: every 8-bit walk stops here at the latest (upstream: 32) */
...
void InsertBreath()
{
    ...
    X++;
    mem55 = 0;
    unsigned mem66 = 0;                            /* not 8-bit: must not wrap to 0 */
    while(1)
    {
        //pos48440:
        if (mem66 > 254) return;
        ...
        if (mem54 == 255) mem54 = X;               /* no word boundary in 232 frames: break here (upstream wrote entry 255) */
        X = mem54;
...
// SET THE STRESS FOR THE PRIOR PHONEME
        if (position) stress[position-1] = Y;   /* a stress digit before the first phoneme wrote stress[-1] */
    } //while
}
...
void Code41240()
{
    unsigned pos=0;                                /* not 8-bit: pos += 3 must not wrap to 0 */
...
// Look at prior phoneme
        X--;
        A = phonemeindex[(unsigned char)(pos-1)];
        //pos41567:
        if (A == 69)                    // 'T'
        {
// Change T to CH

            phonemeindex[(unsigned char)(pos-1)] = 42;
            goto pos41779;
        }
...
// Prior phonemes D?
        ...
        {
// Change D to J
            phonemeindex[(unsigned char)(pos-1)] = 44;
...
// If prior phoneme does not have VOWEL flag set, move to next phoneme
            if ((flags[phonemeindex[(unsigned char)(pos-1)]] & 128) == 0) {pos++; continue;}
...
// If prior phoneme is not G, move to next phoneme
            if (phonemeindex[(unsigned char)(pos-1)] != 60) {pos++; continue;}
...
//       UW -> UX
        ...
        {
// ALVEOLAR flag set?
            Y = phonemeindex[(unsigned char)(X-1)];
            A = flags2[Y] & 4;
...
// If prior phoneme is not a vowel, continue processing phonemes
        if ((flags[phonemeindex[(unsigned char)(X-1)]] & 128) == 0) {pos++; continue;}
...
int debugX = X;
        ...
        if ((flags2[index] & 16) != 0)
        {
            ...
            index = X ? phonemeindex[(unsigned char)(X-1)] : 255;   /* no prior phoneme (read phonemeindex[-1] upstream) */
