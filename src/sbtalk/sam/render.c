...
{
    unsigned char ary[5]; ary[0] = ary[1] = ary[2] = ary[3] = ary[4] = A;
...
// reset at the beginning of each glottal pulse.
    ...
    while(1)
    {
        ...
        } else
        {
            ...
            unsigned char ary[5];
            unsigned int p1 = (unsigned)phase1 * 256u; // Fixed point integers because we need to divide later on
            unsigned int p2 = (unsigned)phase2 * 256u;
            unsigned int p3 = (unsigned)phase3 * 256u;
            int k;
            for (k=0; k<5; k++) {
                ...
                ary[k] = mux;
                p1 += (unsigned)frequency1[Y] * 64u; // Compromise, this becomes a shift and works well
                p2 += (unsigned)frequency2[Y] * 64u;
                p3 += (unsigned)frequency3[Y] * 64u;
            }
