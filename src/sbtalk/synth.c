...
void synth_main(void)
{
    ...
    synth_stage = 11;
    setjmp(restart);
    ring_abort_ack(); utt_len = 0; in_cmd = 0;
    for (;;) {
        if (ring_abort) { ring_abort_ack(); utt_len = 0; in_cmd = 0; }
        c = text_get();
        if (c < 0) {
        ...
        if (in_cmd) {
            ...
            else if (c == 1) { cmd_val = 0; cmd_has_val = 0; }                  /* ^A^A: the command starts over */
            else if (c == 24) { in_cmd = 0; utt_len = 0; }                      /* ^X inside a command is still a flush */
