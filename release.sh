#!/usr/bin/env bash
# Build all six artifacts into dist/ and (with --publish) create a GitHub release with them.
...
run/mktalkdisk.sh
run/mksrcdist.sh
ls -la dist/a11y386.zip dist/espk386.zip dist/talkdisk.img dist/talkpc.img dist/talkpc.zip dist/a11y-src.zip
if [ "${1:-}" = "--publish" ]; then
  tag="${2:-v$(date +%Y.%m.%d)}"
  gh release create "$tag" dist/a11y386.zip dist/espk386.zip dist/talkdisk.img dist/talkpc.img dist/talkpc.zip dist/a11y-src.zip \
