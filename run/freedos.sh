...
# Env: SOUND=sb16|hda|both, DISK=path to installed hard-disk image (default: boot the LiveCD), SHARE=dir, CPU=486|pentium,
...
# SOUND=sb16 (default) | hda (Intel HD Audio ICH9 + duplex codec, for the SBEMU stack) | both
case "${SOUND:-sb16}" in
  hda) SOUND_ARGS=(-device ich9-intel-hda,id=hda0 -device hda-duplex,bus=hda0.0,audiodev=snd0);;
  both) SOUND_ARGS=(-device sb16,iobase=0x220,irq=5,dma=1,dma16=5,audiodev=snd0 -device ich9-intel-hda,id=hda0 -device hda-duplex,bus=hda0.0,audiodev=snd0);;
  *) SOUND_ARGS=(-device sb16,iobase=0x220,irq=5,dma=1,dma16=5,audiodev=snd0);;
esac
